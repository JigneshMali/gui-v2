...
Page {
    ...
    property VeQuickItem sfkV3H2FirmwareReleaseText: VeQuickItem {
        uid: root.bindPrefix + "/H2Device/SFKV3H2FirmwareReleaseText"
    }
    property VeQuickItem sfkV3H2Url: VeQuickItem {
        uid: root.bindPrefix + "/H2Device/SFKV3H2Url"
    }
    property VeQuickItem sfkV3H2DownloadProgress: VeQuickItem {
        uid: root.bindPrefix + "/H2Device/SFKV3H2DownloadProgress"
    }
    property VeQuickItem sfkV3H2DownloadConfirm: VeQuickItem {
        uid: root.bindPrefix + "/H2Device/SFKV3H2Downloadconfirm"
    }
    ...
    GradientListView {
        model: VisibleItemModel {
            ...
            ListText {
                text: qsTr("Version")
                secondaryText: sfkV3H2version.value
                preferredVisible: pageVisible && sfkV3H2version.value !== ""
            }

            ListButton {
                text: qsTr("Download %1").arg(sfkV3H2version.value)
                secondaryText: sfkV3H2DownloadCompleted.value === 1
                        ? qsTr("Downloaded")
                        : sfkV3H2DownloadProgress.value > 0
                            ? qsTr("Downloading... %1%").arg(sfkV3H2DownloadProgress.value)
                            : qsTr("Download")
                preferredVisible: pageVisible && sfkV3H2version.value !== ""
                onClicked: {
                    if (sfkV3H2DownloadProgress.value === 0 && sfkV3H2DownloadCompleted.value !== 1)
                        sfkV3H2DownloadConfirm.setValue(1)
                }
            }
            ...
            ListText {
                text: sfkV3H2FirmwareReleaseText.value
                preferredVisible: pageVisible && sfkV3H2version.value !== ""
            }
        }
    }
